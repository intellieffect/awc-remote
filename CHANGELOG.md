...
- Fix `capture`, `expect` and `stable` intermittently hanging until their deadline against a polling server (x11vnc) that answers with a pointer-position or desktop-size update while the client is waiting for a screen change. The client re-requested such an empty update non-incrementally, which tells the server the client lost its contents and to resend everything (RFC 6143 7.5.3), abandoning the incremental wait and letting the awaited change be dropped. The re-request now keeps the incremental mode of the refresh it is continuing (@bruce, INT-5339)
- The OS-hosted server tests (`test_server_compat_native.py`) register only on a GitHub-hosted CI runner (`RUNNER_ENVIRONMENT=github-hosted`); a local `unittest discover` over `tests/functional` used to dial whatever listened on this machine's port 5900, which on a Mac with Screen Sharing on is the developer's own desktop (@bruce, INT-5318)
- Add `awc-remote`, a JSON command line, and the `vncdotool.reliability` package behind it: `probe` bounds connect, authenticate and first-frame as separate stages with their own timeouts and outcomes; `act` sends one input and settles a receipt of `verified`, `sent`, `unknown` or `failed` against a caller-chosen predicate on a frame received afterwards, never resending on `unknown`; frames carry receive time, sequence, generation and size, with an all-black frame flagged suspect rather than read as a lock and identical pixels reported as static rather than stale; `lease` gives cooperative, token-checked, expiring exclusive leases per target, and `--lease-owner`/`--lease-token` run `probe`/`act` under one, renewed to outlast the run and re-checked before any input is written. The password comes from the environment or a file, never an argument. See `docs/reliability.md` (@bruce, INT-5318)
